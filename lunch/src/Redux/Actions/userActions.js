...
import { APP_SUCCESS } from "./../Constants/userConstants";
...
// apply
const applyUser = (form) => async (dispatch) => {
  try {
    dispatch({ type: userConstants.APPLY_REQUEST });
    const config = {
      headers: {
        "Content-Type": "application/json",
      },
    };
    const { data } = await Axios.post("/users/apply", form, config);
    dispatch({
      type: userConstants.APPLY_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
      type: userConstants.APPLY_FAIL,
      payload:
