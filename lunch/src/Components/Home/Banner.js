    ...
    >
      {Movies?.slice(0, 8).map((movie, index) => (
        <SwiperSlide key={index} className="relative rounded overflow-hidden">
          ...
          />
          <div className="text-white absolute linear-bg xl:pl-52 sm:pl-32 pl-8 top-0 bottom-0 right-0 left-0 flex flex-col justify-center lg:gap-8 md:gap-5 gap-4">
            ...
            <div className="flex gap-5 items-center">
              <Link
                // to={`/watch/${Movies?._id}`}
                to={`/movie/${movie?._id}`}
                className="bg-primary hover:text-main transitions text-white px-8 py-3 rounded font-medium sm:text-sm text-xs"
              >
                Apply
              </Link>
              <button
