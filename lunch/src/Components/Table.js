        ...
        {admin ? (
          ...
          <>
            <button
              ...
              className="border border-border bg-text flex-rows gap-2 text-border rounded py-1 px-2"
            >
              Apply <FaCloudDownloadAlt className="text-green-500" />
            </button>
...
//   table
function Table({ data, admin, onDeleteHandler, downlodVideo, progress }) {
  return (
    <div className="overflow-x-scroll overflow-hidden relative w-full">
      <table className="w-full table-auto border border-border divide-y divide-border">
        ...
        <tbody className="bg-white divide-y divide-gray-800">
          {data.map((movie, i) =>
            Rows(movie, i, onDeleteHandler, admin, downlodVideo, progress)
          )}
        </tbody>
      </table>
    </div>
